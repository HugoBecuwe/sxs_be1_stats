...
The estimator can't be used to estimate an optimal bandwidth as it is an increasing function of hs.
...
ON PEUT POTENTIELLEMENT TROUVER UN OPTIMUM SUR LE POINT D'INFLEXION A 0.05
...
When the bandwidth is rather large, then the gaussian kernel is more centered. That means that for a given $x$, more weight will be given to the $X_i \sim x$ and fewer to the other $X_i$.

So when applied to $x \sim 0$ or $x \sim 1$ the estimator will be mostly calculated on zones where there is very few information as the function is close to zero.
...
fHat_LP <- function(bandwidth, n, sigma2, f, degree) {
    ...
    #locpoly(X, Y, degree = degree, bandwidth = bandwidth, gridsize = length(X))
    fit$y
} 
...
plot(X, MSE(best, n, sigma2, f, 50), col = 1, main = "Previous MSE with optimal bandwidth")
legend("top", lgd, col = 2:4, lty = 1)
abline(h=0, lty = 2)

for ( i in 1:length(degree_list) ){
  
  plot(X, MSE_LP(optim_bandwidth_list[i], n, sigma2, f, 50, degree_list[i]), col = 1 + i,
...
These plots are using the same y scale so that we can compare the boundary effects.
Even though we can still see some increase in the MSE at the boundaries, it seems that increasing the degree of the local polynom used is decreasing the boundary effect.
